...
interface AssignJobPOModalProps {
  open: boolean;
  onClose: () => void;
  jobId: string;
  jobPartNumber: string | null;
  jobTitle: string;
  // Prefills the search — e.g. a PO number already typed into the job's
  // free-text "PO Number" field before anyone actually linked it.
  initialQuery?: string;
  onAssigned: (result: { po_id: string; po_number: string; customer: string; matched_line_item_id: string | null }) => void;
}
...
export default function AssignJobPOModal({ open, onClose, jobId, jobPartNumber, jobTitle, initialQuery, onAssigned }: AssignJobPOModalProps) {
  ...
  // A job's "Part Number(s)" field can hold a comma-separated list — use the
  // first token as the line-match hint, same as a vehicle's single part.
  const partHint = jobPartNumber?.split(',')[0]?.trim() || null;
  ...
  const fetchPOs = useCallback(async (q: string) => {
    setSearching(true);
    try {
      const res = await fetch(`/api/graphics-jobs/assign-po?q=${encodeURIComponent(q)}`);
  ...
  const handleAssign = async (po: PO) => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/graphics-jobs/assign-po', {
        method: 'POST',
      ...
      const data = await res.json();
      if (data.success) {
        onAssigned({ po_id: po.id, po_number: data.po_number, customer: data.customer, matched_line_item_id: data.matched_line_item_id ?? null });
        onClose();
      } else {
        setError(data.error || 'Failed to link PO');
      }
    } catch {
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div>
              <div style={{ fontSize: '15px', fontWeight: 800, color: 'var(--text-body)' }}>Link Purchase Order</div>
              <div style={{ fontSize: '11px', color: 'var(--text-body)', marginTop: '2px' }}>
                {jobTitle}{partHint ? ` · Part: ${partHint}` : ''}
              </div>
            </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {pos.map((po) => {
            ...
            const matchingLine = partHint
              ? po.po_line_items?.find((l) => l.part_number?.toLowerCase() === partHint.toLowerCase())
              : null;
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '6px' }}>
                      ...
                      <div style={{ marginTop: '6px' }}>
                        {po.po_line_items.slice(0, 3).map((li) => (
                          <div key={li.id} style={{
                            ...
                            <span style={{
                              color: li.part_number?.toLowerCase() === partHint?.toLowerCase() ? '#4ade80' : 'var(--text-body)',
                              fontWeight: li.part_number?.toLowerCase() === partHint?.toLowerCase() ? 700 : 400,
                            }}>
                              {li.part_number}
                            </span>
                            <span>·</span>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: matchingLine ? 'rgba(74,222,128,0.15)' : 'rgba(96,165,250,0.12)',
                      color: matchingLine ? '#4ade80' : '#60a5fa',
