...
const VEHICLE_STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  ...
  delivered: { label: 'Delivered', color: '#6b7a8d', bg: 'rgba(107,122,141,0.1)' },
};

const GRAPHICS_STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  pending: { label: 'Pending', color: '#6b7a8d', bg: 'rgba(107,122,141,0.1)' },
      ...
      {(tab === 'all' || tab === 'upfit') && vehicleJobs.length > 0 && (
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {vehicleJobs.map(job => {
              const status = VEHICLE_STATUS_MAP[job.status] || { label: job.status, color: '#6b7a8d', bg: 'rgba(107,122,141,0.1)' };
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {graphicsJobs.map(job => {
              const status = GRAPHICS_STATUS_MAP[job.status] || { label: job.status, color: '#6b7a8d', bg: 'rgba(107,122,141,0.1)' };
