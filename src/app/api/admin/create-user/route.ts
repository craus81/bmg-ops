...
export async function POST(req: NextRequest) {
  try {
    ...
    if (profileError) {
      ...
      // Don't fail — auth user exists, profile can be fixed manually
    }

    // 3. Send invite email if requested
    if (sendInvite) {
      try {
        // Use the unified notify system if SendGrid is configured
            ...
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; background: #0a1017; color: #e8ecf1; padding: 32px; border-radius: 16px;">
              ...
              <div style="background: #141e2b; border: 1px solid #1e2d3d; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                ...
                <p style="color: #c8d6e5; line-height: 1.6; margin: 0 0 16px;">
                  Your FleetSuite account has been created. Here are your login credentials:
                </p>

                <div style="background: #0f1720; border-radius: 8px; padding: 14px; margin-bottom: 16px;">
              ...
              <a href="${appUrl}" style="display: block; text-align: center; padding: 14px; background: #3b82f6; color: #ffffff; font-weight: 800; font-size: 14px; border-radius: 10px; text-decoration: none;">
        ...
        console.warn('Invite email failed (SendGrid may not be configured):', emailErr.message);
        // Don't fail the whole request — user is created, email is just a bonus
      }
    }
    ...
    return NextResponse.json({
      success: true,
      userId,
      message: `User ${fullName} created successfully${sendInvite ? ' — invite email sent' : ''}`,
    });
  } catch (err: any) {
