 ...
 * modeled on packing-list-pdf.ts. ONE pure builder serves every output:
 * the in-app view/print (src/lib/statement-pdf.ts, client) and the PDF
 * copy attached to the statement email (src/lib/statement-pdf-server.ts),
 * so what staff open is byte-for-byte what the customer receives.
 *
 * Pure — no window/DOM access, so a server route can generate the bytes.
 *
...
export interface StatementPdfData {
  ...
  to?: string | null; // ISO
  /** Company letterhead — same source as the wrap quote's header. The
   *  logo must already be inlined (jsPDF can't fetch). */
  letterhead?: { company: CompanyProfile | null; logoDataUrl: string | null } | null;
}
...
/** File name for the generated PDF, shared by the view and email surfaces. */
export function statementPdfFilename(customer: string, date = new Date()): string {
  return `Statement_${customer.replace(/[^\w\- ]+/g, '').trim().replace(/\s+/g, '_') || 'Customer'}_${date.toISOString().slice(0, 10)}.pdf`;
}
