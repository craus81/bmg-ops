// Pure positional-text → PO parsing for Masterack PO PDFs.
// Extracted from parsePO.ts (which owns the pdfjs-dist text extraction) so
// the column-mapping logic can be tested without a PDF engine.
...
export function parsePOFromItems(allItems: TItem[], debug = false): ParsedPO {
  // Build debug output
  let debugText = '';
  if (debug) {
