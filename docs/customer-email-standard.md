...
| Estimate **internal review** (`/estimates` → Send to a Teammate for Review) | `EmailComposeModal` | **The one INTERNAL flow on this screen.** Goes to a BMG teammate, not the customer: same estimate document and same auto-attached PDF copy, but the CTA opens the estimate in FleetView (`/api/estimates/[id]/send-for-review`, kind `estimate_internal_review`) instead of a customer approval link, and no token is minted or status moved. The To dropdown lists internal staff (`contactsLabel`), and the **first BMG address in To is the reviewer of record** — they get the review task (`estimate_review_requested`) and the Approve / Send back / Pass on buttons on the estimate. Decisions go back to the requester as `estimate_review_update`. Sending to the customer while a review is pending resolves it as approved (migration 316). |
...
| Invoice emails (`EmailInvoicesModal`) | Own modal, fits the standard | Multi-To, message, bcc-me, invoice-PDF attachments with verify + view, test-send, delivery tracking. Migrate to the shared component if it's ever rebuilt. Entry points: the Invoices hub (per customer / per invoice / post-scan-invoicing), graphics pages, `/admin/pos/[id]`, `/admin/scans` — and the vehicle **completion modal**'s ✉ Email button next to each billed sales order's Print button (R3-14b: completion could print an invoice but not send it). |
...
| Segment email blast (`/admin/prospects` → Email this segment) | `EmailComposeModal` | Full standard + a Subject input and the audience breakdown in the intro slot (`/api/prospects/segment-email`, kind `segment_blast`, admin/sales). **One individually-addressed send per recipient** — never one email with many recipients: a shared To line shows every customer who else we quoted, and a shared Bcc still lands as obvious bulk mail. `prospects.email_campaign` gates the send whether or not the sender ticked the campaign filter; records without it are dropped and named in the compose screen, with the count. Merge fields (`{{company}}`, `{{contact_name}}`, `{{contact_first_name}}`) resolve per recipient; a recipient missing a field the template uses is skipped and listed rather than sent “Hi ,”, and an unrecognised `{{token}}` is a hard error before anything is previewed or sent. The To field NARROWS the audience (removing an address drops that recipient) and cannot widen it — an address with no record behind it has nothing to merge from. Capped at 100 per run; failures come back named, not counted. Every successful send writes a `prospect_activities` row on the record. |
