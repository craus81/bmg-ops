...
- ✅ **MAJOR — Duplicate guard is exact-name-only**, client-side, and inconsistent
  ...
  first estimate is pushed/converted; no schema change — *superseded
  2026-09-02 by #772: NetSuite-customer-on-create is now the DEFAULT via a
  ticked checkbox, with the lead tier as the untick/failure fallback; see
  §7.4*) and the
...
- ✅ **MAJOR — Install guides bypass the feature system** (raw role flags), and a
  ...
  DB policies (233/234/248) already treat super_admin ⊇ admin. Decided and
  shipped 2026-09-07, #827: super_admin IS a strict superset of admin
  app-wide — see §7.5.)_
...
- ✅ **MAJOR — No job-level labor capture.** Nothing links time entries or shifts to
  ...
  _(Decided and shipped 2026-09-07 — #831 + migration 269, job costing only
  by owner decision: `'shop'`-context `work_shifts` bound to a check-in,
  started/stopped from the pick-list's Labor Timer card with crew tags;
  completion and a daily sweep auto-close forgotten timers, flagged
  approximate; presence-overlap hours × the blended Settings rate land in
  the vehicle-margin report. Never pay — install_credits and the punch
  clock are untouched.)_
...
visits) — with #650/#632/#634/#635/#638/#663 all re-verified at HEAD. The
stage's last open build, R3-21 job-level labor capture, shipped 2026-09-07
(#831 + migration 269 — see §7.5): the stage is closed in full.
...
owns the wider payment-surface decision *(both since closed — R3-14's
build 2026-09-07 via #810/#811, R3-22 the same day via #834/C2)*.
...
19. ❌ **R2 objects are world-readable.** A gated download route exists and
    ...
    *(Decided and shipped 2026-09-07 — #834, owner call C2. The
    email-breakage trap dissolved: exactly the email-embedded imagery
    (logos/diagrams under `vehicle-templates/*`, part shots under
    `photos/parts/*`) stays on an edge-allowlisted public path; every other
    read is auth-gated or presigned, and `createSignedUrl` now returns the
    auth-gated stream. The flip is a one-rule Cloudflare runbook action —
    `docs/r2-private-flip.md`.)*
...
21. ❌ **Job-level labor capture** — nothing links shifts or time entries to a
    ...
    job the whole app exists to invoice. *(Shipped 2026-09-07 — #831 +
    migration 269, pick-list labor timers priced into the margin report;
    see §7.5.)*
...
| 19 | ~~Open~~ **Fixed 2026-09-07 (#834)**: every read outside the C2 email-imagery allowlist is auth-gated or presigned, storage reads are caller-tiered, and the flip is a one-rule Cloudflare runbook action (`docs/r2-private-flip.md`). |
...
| 21 | ~~Open~~ **Fixed 2026-09-07 (#831, migration 269)**: `work_shifts.fleet_checkin_id` + the `'shop'` context tie a timer to its check-in; pick-list start/stop, auto-close honesty, blended-rate costing in the margin report. |
...
quick answer. *(It got that conversation 2026-09-07 and closed as C2 with
PR #834; see the item 19 annotation above and `docs/r2-private-flip.md`.)*
...
- ✅ `wrap-quote/create-customer` returns **502 "create failed" on a
  ...
  _(Closed 2026-09-02, #805 with R3-9's ship: `createCustomerOrLead`
  recovers an unparseable Location header by newest-id name lookup, and
  the unrecoverable remainder says the customer exists and steers to the
  search instead of inviting the retry.)_
...
- ✅ `promote-prospect` is a read-then-write race: two concurrent pushes of
  ...
  _(Closed 2026-09-02, #805 + migration 254: the atomic promotion claim —
  claim → create → stamp retires it, release on failure, 15-min stale
  reclaim, never a falsy stamp.)_
...
- ✅ The main CRM create (`admin/prospects/page.tsx:381`) and `addToCrm`
  ...
  _(Closed 2026-09-02, #805 — both creates go through `POST /api/prospects`
  so the guard is enforced, not previewed; the page keeps its dialogs, a
  server 409 replays them, and Add Record becomes a
  `fromNetsuiteCustomerId` mode that copies identity from the customers
  mirror server-side.)_
- ✅ `PUT /api/prospects` is `.passthrough()` + `requireStaff`, no audit: any
  ...
  _(Closed 2026-09-02, #805 + migration 254 — `.strict()` whitelist,
  `netsuite_id` changes require an admin and audit-log old → new, and a
  233-shape trigger makes every identity column service-role-only at the
  DB, browser consoles included.)_
- ✅ Deleting a customer: the mirror delete is unchecked and FK-blocked by
  ...
  _(Closed 2026-09-02, #805 — checked delete with a kept-inactive fallback
  when FKs block it, the response/audit report which, and the confirm
  finally names the file cascade.)_
- ✅ The credit-app reviewer's "candidate matches" honors `*` as a wildcard
  ...
  _(Closed 2026-09-02, #805 — `*` stripped, the attacker-supplied email arm
  participates only when wildcard/syntax-free, and `checkRateLimit` buckets
  no-IP requests instead of waving them through.)_

**7.2.5 STATUS (2026-09-02): CLOSED** — creates enforce the guard, the
NetSuite linkage is service-role-only + admin/audited via the API,
deletes are checked and honest, and the reviewer's matcher can't be
steered (#805 + migration 254). The promote race and the create-customer
502-on-success closed with the same ship (see 7.2.4).
...
- ✅ Four broken deep-link contracts: CNI chat mentions store the *sender's*
  ...
  _(Closed 2026-09-02, #770 — R3-5: `cniJobLinkFor(roles, jobId)` routes
  each mention recipient to their own portal, applied per recipient in the
  mentions route; the ready-for-install gate admits the shop/field techs
  notify-ready targets; thread reply emails CTA to `customerPortal()` with
  a reply-to-this-email footnote; scanPhotos died in #761; and the
  hand-built-URL migration is finished across #767/#768/#770 — a sweep for
  hand-built notification/mention URLs now comes back empty.)_

**7.2.7 STATUS (2026-09-02): CLOSED** — the notify layer respects
preferences with an explicit un-silenceable tier (#768), CNI audiences are
feature-scoped and per-VIN fan-out is collapsed (#768), and every deep-link
contract holds: cross-portal recipients land in their own portal, every
notification audience can open its destination, and all producer URLs come
from the canonical builders (#770).
...
5. **R3-5 · Deep links** — `cniJobLinkFor` per-recipient; widen the
   ...
   _(✅ Shipped 2026-09-02, #770: per-recipient `cniJobLinkFor` wired into
   the mentions route; the ready-for-install gate widened to its notified
   audience (shop/field techs); thread replies CTA to `customerPortal()`;
   scanPhotos had died in #761; the hand-built-string migration completed
   across #767/#768/#770 — the sweep now finds zero.)_
...
8. **R3-8 · Money-path idempotence** — unique indexes on every
   ...
   create-customer's 502-on-success, promote-prospect's race~~ _(both
   closed 2026-09-02 with R3-9's ship, #805 + migration 254)_, and the
   unique netsuite_*_id indexes — deliberately not
...
9. **R3-9 · CRM lifecycle** — route the CRM creates through
   ...
   _(✅ Shipped 2026-09-02 in full, #805 + migration 254 — plus the two
   §7.2.4 paths that rode along: the promote race closed with the atomic
   claim, and create-customer's 502-on-success replaced by id recovery +
   an honest may-exist answer. §7.2.5 closed outright.)_
...
19. **R3-19 · The job-margin report** — parts cost (receipts/bills) +
    labor (lit up by R3-21, #831) against the invoice, per vehicle;
    cycle-time and damage-volume reports from data that already exists.
20. **R3-20 · Floor ergonomics** — graphics lane on the pick-list;
    pickup-path billing prompt; purchasing/receiving count badges.

### Carried decisions

21. **R3-21 (= item 21) · Job-level labor capture — ✅ SHIPPED 2026-09-07
    (#831 + migration 269).** The owner call narrowed the touch-map: job
    COSTING only, so the `CompletionRef`/credits half was deliberately not
    built — `'shop'`-context `work_shifts` gain `fleet_checkin_id` (named
    CHECKs replace 110's unnamed inline ones), the pick-list runs
    start/stop with crew tags over the shop roster, forgotten timers
    auto-close flagged approximate (completion + a daily sweep at a 12h
    cap), and one blended cost rate (Settings, super-admin, audited)
    prices presence-overlap hours into the margin report. The browser-only
    `time_entries` day-clock stays its own system, untouched.
22. **R3-22 (= item 19) · R2 goes private — ✅ SHIPPED 2026-09-07 (#834,
    owner call C2).** The tiers landed leaner than the checklist priced
    them: (A) collapsed to ONE shim change — `getPublicUrl()` returns the
    auth-gated same-origin stream (the `/api/storage` reliability worry
    didn't hold: same-origin `<img>`/pdfjs ride the session cookie like
    every other app call); (B) the approval pages/PDF already presigned
    (#746/#688) and the rest now always does; (C) resolved by decision,
    not code — the C2 edge allowlist keeps exactly the email-embedded
    imagery public (`vehicle-templates/*`, `photos/parts/*`), so no sent
    email breaks and nothing irreversible happened; (D/F) swapped and
    deleted; (E) no backfill — stored `public_url` rows became legacy key
    carriers and reads presign. New beyond the checklist: the generic
    storage routes tier callers (staff / installer floor prefixes /
    customers denied), which is what makes the flip worth flipping. The
    flip itself: one Cloudflare rule, `docs/r2-private-flip.md`.
...
## 7.4 Remainder re-verification — 2026-09-06

Parallel sessions shipped 33 PRs (a range from #772 through #804) between
the R3-5 close and this check, so every roadmap item still marked open was
re-verified against the code as it stands (three parallel code probes at
the post-#806 HEAD; every verdict below carries the mechanism, not the
doc's memory). Each open item also gets its **proposed fix in plain
language** — what we'd actually build, described for a human, not a
compiler.

**Closed or changed by the parallel work:**

- **R3-16's link-by-id half is DONE** — estimates now carry
  `prospect_id` (migration 251) and the promote path resolves the linked
  lead by id first, name-match only as fallback. Pushing an estimate
  promotes *that* lead, not whichever record shares its name.
- **The lead tier was deliberately reversed** (owner decision 2026-09-02,
  PR seven-seventy-two): creating a record now creates the NetSuite
  customer in the same click by default, via a ticked checkbox. The lead
  tier survives underneath — untick the box, or hit a NetSuite failure,
  and you have a plain lead the record page can promote later. Composes
  cleanly with R3-9's guarded create and the promotion claim.
- Adjacent ships worth knowing: per-line tax parity with NetSuite, a PDF
  copy on every transaction email, the SO mirror rebuilt (cursor paging +
  retries — it actually fills now), an estimate→SO change-push prompt
  (migration 259), the in-app camera session, photo retention past
  auto-archive, the customer PO-status portal (migration 260), and the
  fulfil-before-invoice gate.

**Still open, sharpest first — with the proposed fix:**

1. **Estimate push write-back (R3-8's last code item) — ✅ SHIPPED
   2026-09-07 (PR #808, migration 262).** The push route's four stamps
   (the NetSuite id after create, the customer id, the update-branch
   status, the delete clear) were all fire-and-forget, and
   create-vs-update was decided from one stale read. If the stamp
   failed, the app said "pushed!" anyway and the NEXT push created a
   second real NetSuite estimate.
   *Fix in plain language: treat pushing like converting. Before calling
   NetSuite, claim the estimate so a second click waits its turn; after
   NetSuite answers, write the id down and CHECK that the write worked —
   if it didn't, say so instead of celebrating. The exact pattern already
   lives in convert-to-so; this is copying it across the hallway.*
   *Shipped exactly that: `estimates.push_claimed_at` is claimed before
   the NetSuite call (the second click gets a 409, a claim older than 15
   minutes is stale and reclaimable, and a schema cache that hasn't seen
   the migration degrades to the old behavior instead of bricking
   pushes). The create write-back is conditional (first writer wins),
   checked, and never falsy (`created-id-unknown` sentinel keeps the
   create-guard armed when NetSuite returns no id); a failed
   create-stamp HOLDS the claim — the only thing standing between a
   retry and a duplicate — and the push dialog now surfaces the warning.
   The customer stamp, update stamp, and delete's link-clear are checked
   too, and every early return releases the claim.*
2. **The per-SO invoice trap — ✅ SHIPPED 2026-09-07 (PR #808,
   migration 261).** The completion modal showed an "Invoice SO …"
   button for every linked sales order, but the check-in stored exactly
   one invoice number and the API refused a second — so a vehicle with
   three SOs could bill one of them, behind a UI offering all three.
   *Fix in plain language: either finish the job — store one invoice
   PER sales order (a small join table instead of the single column) so
   every button works — or hide the extra buttons until that exists.
   Finishing it is the right call: the buttons are already there because
   somebody needs them.*
   *Shipped the finish-the-job option: `fleet_checkin_invoices`, one row
   per (check-in, sales order), whose UNIQUE pair IS the claim — the row
   is inserted before the NetSuite call (a concurrent second click turns
   away on the unique violation; a stale claim is taken over after 15
   minutes), stamped with the invoice on success, deleted on failure.
   Fulfil-first and fail-closed behavior unchanged. The modal now shows
   each order's own billed chip with its own Print/PDF button, or its
   own Invoice button. The vehicle's FIRST invoice still lands on the
   legacy scalar (the archived card, unpaid tile, and AR payment sync
   read it); a check-in invoiced before the ledger existed carries a
...
3. **Invoice send step + never-invoiced tile (R3-14 b/c) — ✅ SHIPPED 2026-09-07 (PR #811).**
   ...
   *Fix: put the existing invoice-email screen (the one the Invoices hub
...
5. **Readiness that pushes (R3-12) — ✅ SHIPPED 2026-09-07 (PR #812; the multi-PO join table followed the same day, #824/#825).** One-click "request the
   ...
   ETA changes (the date updates silently today), and multi-PO project
   links (first PO wins the single column).
   *Fix, in order of value: notify the project's people when the ETA
   ...
   a small join table and is the biggest lift — shipped on request as
   #824/#825 (migration 267), see §7.5.*
...
7. **Promote carries everything (R3-16a) — ✅ SHIPPED 2026-09-07 in full (contacts PR #814; the notes half decided and shipped the same day, #829 + migration 268 — see §7.5).** Promotion sends only
...
10. **Job-margin report (R3-19) — ✅ SHIPPED 2026-09-07 (PR #818; labor column lit up by R3-21, #831).** Installer-cost and
...
12. **Unique NetSuite-id indexes — ✅ SHIPPED 2026-09-07 (PR #819, migration 264).** Still no
    ...
    production doesn't hold it from the hand-migrated era, every
    promotion's mirror write fails quietly and the 2-hour sync papers
    over it.
    *Fix: first LOOK — check production for that constraint and for
    existing duplicate ids (a read-only query); then add the unique
    indexes with a dupe-report step, never blind (a blind build on dirty
    data blocks every deploy).*
13. **Unchanged decisions and builds:** R3-21 labor capture and R3-22
    R2-goes-private both stand with their written decision packages
    (the R2 flip hasn't started — ~59 public-URL usages live; only the
    new PO portal presigns) *(both decided and shipped 2026-09-07 —
    R3-21 as #831 + migration 269, job costing only; R3-22 as #834,
    owner call C2, the flip now a one-rule Cloudflare runbook action.
    See §7.5 — nothing remains with the owner but that click)*. The requireAdmin-excludes-super_admin
    question is still the owner's call *(decided and shipped 2026-09-07,
    #827 — super_admin ⊇ admin app-wide; see §7.5)*. Roll-nesting for
    production and the CNI install checklist remain the two open floor
    builds.
...
- **Item 5 (PR #812):** a PO's ETA CHANGE now notifies each affected
  ...
  NetSuite. The multi-PO join *table* shipped later the same day
  (#824/#825, below).
...
- **Item 7 (PR #814):** promotion walks the record's contacts and
  ...
  counted, surfaced, and retryable from the Contacts card. Notes stayed
  deliberately unpushed pending the owner decision the audit called
  for — taken and shipped later the same day (#829, below).
...
- **Item 10 (PR #818):** `/admin/reports/vehicle-margin` — each invoiced
  ...
  *(landed 2026-09-07, #831 — the column now prices timer hours)*.
...
**And the two floor builds landed the same day:**

- **CNI install checklist (PR #821, migration 265):** `cni_job_tasks` —
  coordinator-authored steps (prep, wiring, torque, cleanup, sign-off)
  the installer checks off, per the 226/253 write-lockdown (a 253-style
  sweep guarantees no non-SELECT policy; every write rides the
  service-role `/api/cni/job-tasks`). Required tasks 409-gate Mark Job
  Complete with the open steps named (admin bypass mirrors the photo
  gate; task-less jobs pass vacuously) and join the admin closure
  check; one `cni_checklist_complete` ping fires when an installer
  closes the last required step. Both portals render the list
  browser-side via the company-scoped SELECT.
- **Roll-nesting for production (PR #822, migration 266):** the Roll
  Plan card on the graphics job page runs the wrap-quote nesting engine
  (paneling, MaxRects packer, drag/rotate canvas) over the job's own
  pieces — typed in, or pulled from the linked wrap quote's
  measurements — persists to `graphics_jobs.nesting`, and "Log material
  from plan" writes the computed roll usage (sqft, linear feet, cost
  from the shop's last logged rate when one exists) straight into
  `graphics_job_materials`, ending the manual inventory-blind typing.

**And the deferred big lift closed on request the same evening:**

- **Multi-PO project links (PRs #824 + #825, migration 267):**
  `upfit_project_pos` — one row per (project, vendor PO), referencing
  the mirror row the way `purchase_requests.ordered_po_id` does, and
  backfilled from the request join plus both first-PO-column
  resolutions, so day one it knew everything the system already knew.
  create-po now links EVERY source project (the scalar columns keep
  their first-PO stamp for legacy readers — the 100/261 pattern), and
  staff link/unlink NetSuite-born POs from the project page through
  `/api/upfit-projects/link-po`, resolved by the ETA scan's own
  exact/digits matcher and timeline-noted both ways. The readers
  follow: the Linked Records card lists every linked PO with vendor,
  status chip, and ETA; parts-email-scan propagates ETAs through the
  join table — join rows govern a project that has any (a deliberate
  unlink stays unlinked), the scalar match remains the fallback for
  projects with none, and `parts_eta` becomes the max across the
  project's OPEN linked POs, so the page stops showing a confidently
  wrong date when the late PO isn't the first one, while people on a
  moved PO hear about it even when a later PO still gates the project;
  and the vehicle-margin report counts manually linked POs in each
  project's parts cost. Six new tests pin the ETA semantics.

**And the requireAdmin vs super_admin decision closed 2026-09-07 (#827):**

- **super_admin is a strict superset of admin, app-wide.** The app had
  answered the question both ways: the DB policies (233/234/248/250),
  `is_internal_staff()` (224), and the feature resolver treated
  super_admin as admin-plus, while `requireAdmin`, `requireRole`'s
  auto-pass, `requireStaff`'s role list, the client `isAdmin` flag, and
  ~30 literal `'admin'` checks tested the literal role only — a pure
  super_admin account was 403'd by all 66 requireAdmin routes and by
  requireStaff itself while passing every feature-gated admin tool (the
  Stage 6 install-guide lockout was one bite of this class). One
  canonical `isAdminRole()` predicate (features.ts, shared by api-auth
  and AuthProvider) now decides it everywhere: the three auth helpers,
  View As, every in-route admin check (shifts, scans, earnings, the
  nine CNI actor checks, vehicle-tracking's override), and the admin
  notification audiences (`.eq('role','admin')` filters widened to
  both roles, `getBillingUserIds`, the approval-alert lists). The
  deliberate walls stay narrower: `requireSuperAdmin` (owner-level)
  and `requireFinancials` (super_admin/executive, NOT regular admin).
  Four new tests pin the predicate and the feature tiers.

**And the notes decision closed 2026-09-07 too (#829, migration 268):**

- **CRM notes reach NetSuite.** The record's human-authored activity —
  the composer's call/email/note/meeting entries and voice notes — lands
  on the NetSuite customer as user notes, oldest first, titled by type
  and date with the author in the body. The trap was the discriminator:
  logAuto writes its app bookkeeping as type 'note' too, so migration
  268 adds `auto` (logAuto sets it; a backfill flags historical rows by
  system type, email_log_id, and the known logAuto summary shapes —
  email-send logs and quote responses stay excluded by their own
  markers, keeping those writers safe through schema-cache lag) and
  `netsuite_note_id` so every drain is idempotent (the
  `created-id-unknown` sentinel keeps the guard armed when NetSuite
  returns no id). One capped `pushProspectNotes` drain serves every
  path: promotion pushes the backlog under the brand-new customer and
  reports notes counts beside the contacts counts, the new
  `/api/prospects/sync-notes` route drains a linked record on demand
  (the page fires it after each new activity; a "Sync notes to
  NetSuite" action retries refusals and continues backlogs), and the
  voice-note route pushes inline. Five tests pin the selection
  semantics, the sentinel, the cap, and the schema-grace no-op.
  R3-16a is now closed in full.

- **R3-21 shop labor capture (2026-09-07, #831 + migration 269).** The
  owner decisions: job COSTING only (never pay — shop techs stay on the
  punch clock, install_credits untouched, so the touch-map's
  `CompletionRef`/credits half was deliberately not built), one blended
  cost rate, the `time_entries` day-clock untouched. `work_shifts`
  grows a `'shop'` context bound to one check-in — named CHECKs replace
  110's unnamed inline ones — and the pick-list gets a Labor Timer
  card: internal floor roles only (the API's FIELD_ROLES wall keeps
  external CNI installers out), start/stop with a live elapsed display,
  and crew chips over the shop_tech/field_tech/graphics_production
  roster, hours counted as presence overlap so late joins and early
  departures bill only their window. Timers nobody stopped stay honest:
  completing/shipping the vehicle auto-closes them, a daily sweep
  closes any open past 14h at a 12h cap, both flag `auto_closed`, and
  every surface marks those hours ≈. The blended rate lives in Settings
  (super-admin write, audited, stated as COST not the selling rate;
  admin read via a manifest-listed route, never returned to floor
  clients); the vehicle-margin report subtracts hours × rate per
  vehicle with totals and CSV columns, or shows hours-only with a
  plain note when no rate is set. Eleven tests pin the overlap math,
  rounding/pricing, approx flagging, and close/sweep idempotence.

- **R3-22 R2 goes private (2026-09-07, #834 — owner call C2).** The
  world-readable file store closes without breaking a single sent email:
  the public R2 domain gets edge-limited to exactly the imagery that
  lives inside customer emails (`vehicle-templates/*` logos and coverage
  diagrams, `photos/parts/*` vendor part shots), and the app stops
  depending on it everywhere else. One shim change moves every signed-in
  surface — photo timelines, tracking, pick-list proofs, scan, parts
  catalog, upfit designer, install guides, check-in — onto the
  auth-gated same-origin stream with zero per-page edits; legacy
  DB-stored public URLs (`fleet_checkins.proof_url`) rewrite at render;
  prospect/estimate file lists hand out fresh 1-hour presigns over
  stored rows kept as key carriers; estimate approval artwork is ALWAYS
  presigned (the token page per visit, the approval email at the 7-day
  SigV4 maximum, with the attached PDF carrying proofs beyond); and the
  generic storage routes finally tier their callers — staff read
  broadly, external CNI installers only their floor prefixes with
  cni-photos held to the record-scoped #765 routes, customer-only
  accounts denied outright, `signed-documents` denied to everyone.
  Seven tests pin the access matrix. Fully reversible and behaviorally
  inert until the flip: one Cloudflare WAF rule, written up with
  verification and rollback in `docs/r2-private-flip.md` (including the
  r2.dev-domain caveat and the optional full-C1 path later).

**Nothing remains open.** Every finding, decision, and build in this
