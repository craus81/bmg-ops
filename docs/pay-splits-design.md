...
**Status:** All three phases are implemented. Phase 1 (companies + tracking):
...
bidding/invites with metric rollups. Phase 3 (individual payouts): migration
`112-payout-mode.sql`, the per-job company/individual payout mode, generated
per-employee payout statements with the draft → approved → billed → paid
lifecycle and NetSuite bill-ID recording, per-person vendor IDs editable on
the company page, and payout-aware job closure.
